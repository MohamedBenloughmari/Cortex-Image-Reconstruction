...
class NeuralEncoder:
    def __init__(self, dx: float, dy: float, dt: float, ds: float = None) -> None:
        ...
        self.optotype_display = None  # for display only
        self.n_steps = None
        self.walk = None
    ...
    def fit(self, optotype: torch.Tensor, blur_sigma: float = 1.5) -> None:
        ...
        # For computation: transpose so axes match (x, y) ganglion grid
        self.optotype_np = np.fliplr(blurred.T)

        # For display: flip vertically to correct MNIST top-left origin
        self.optotype_display = np.flipud(blurred)
    ...
    def compute_activations(self, grid_range=20.0, grid_resolution=40,
        ...
        if type == 'presence':
            ...
            for t in range(n_t):
                ...
                c      = c_raw / g_norm
                lam_on  = np.exp(np.log(lambda0) + np.log(lambda1 / lambda0) * c)
                lam_off = np.exp(np.log(lambda0) + np.log(lambda1 / lambda0) * (1.0 - c))
    ...
    def animate(self, interval=50, save_path=None):
        ...
        # Use optotype_display (correctly oriented) for the left panel only
        opt_img = ax_opt.imshow(
            self.optotype_display,
            extent=[cx0 - half_x, cx0 + half_x, cy0 - half_y, cy0 + half_y],
            origin='lower',
            cmap='gray_r', alpha=0.9, interpolation='bilinear',
        )
        ...
        def _update(frame):
            ...
            walk_line_opt.set_data(self.walk[:frame + 1, 0], self.walk[:frame + 1, 1])
            ...
            if glm_mode:
                ...
                on_scatter.set_array((on_now  / (on_now.max()  + 1e-9)).ravel())
                off_scatter.set_array((off_now / (off_now.max() + 1e-9)).ravel())
            else:
                n_cols  = len(xg)
                ...
                off_scatter.set_array(1.0 - on_vals)

            walk_line_on.set_data(self.walk[:frame + 1, 0], self.walk[:frame + 1, 1])
            walk_line_off.set_data(self.walk[:frame + 1, 0], self.walk[:frame + 1, 1])
...
if __name__ == "__main__":
    ...
    optotype = ds_val[0][0].squeeze(0)  # shape: (28, 28), values in [0, 1]
