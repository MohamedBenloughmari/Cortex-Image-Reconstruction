...
def learn_mnist_dictionary(n_components=64, n_samples=5000,
    ...
    # Use a single global mean so encoder/decoder stay consistent
...
class NeuralEncoder:
    ...
    def _propagate_particles(self, particles, A_hat_np, D, t):
        ...
        for i in range(n_p):
            ...
            log_w[i] = np.sum(
                r_on * np.log(lam_on * self.dt + 1e-12) - lam_on * self.dt
    ...
    def Er_fn(self, S, samples_t, r_on_t, r_off_t):
        """Eq. (14): expected negative log-likelihood under q(X)."""
        nll = 0.0
        ...
        return nll / len(samples_t)

    def Ep_fn(self, A_param, A_anchor, beta):
        """Eq. (15): linearization of -log p(A) = beta * sum |A_k| around A_anchor.

            -E_p(A) = log p(A_anchor) + (A - A_anchor) . d log p / dA |_{A_anchor}
        so
             E_p(A) = beta * sum |A_anchor| + beta * sign(A_anchor) . (A - A_anchor)
        """
        ...
        return -neg_Ep_lin  # i.e. + E_p(A)

    # ------------------------------------------------------------------
    # Optimization of A (eq. 12) via Adam (proxy for FISTA)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Hessian update (eq. 17)
    # ------------------------------------------------------------------
    @staticmethod
    def _hessian_of(scalar_fn, A_anchor):
        """Full Hessian of scalar_fn(A) at A_anchor via autograd."""
    ...
    def _update_hessian(self, H_prev, A_anchor, D_t, samples_t,
                        r_on_t, r_off_t, tau):
        """Eq. (17):  H^{t+1} = exp(-dt/tau) H^t + d^2 E_r^{t+1}/dA^2 |_{A=A^{t+1}}."""
        def Er_of_A(A):
    ...
    def decode(self, D, mean_offset=0.0, n_particles=80, n_samples=30,
        ...
        for t in range(T):
            ...
            # Eq. (17): update Hessian using new A^{t+1}
...
if __name__ == "__main__":
    ...
    D, mean_offset = learn_mnist_dictionary(n_components=16, n_samples=60000,
    ...
    sim = NeuralEncoder(dx=0.2, dy=0.2, dt=0.02, ds=0.3, D_diff=3.0)
    sim.fit(optotype, blur_sigma=0.0)
    sim.simulate_random_walk(T=5)
    sim.compute_activations(grid_range=10.0, grid_resolution=25)
    sim.decode(
        D, mean_offset=mean_offset,
        n_particles=100, n_samples=100,
        beta=0.1, gamma=0.1,
        adam_iter=100, lr=1e-3,
        anchor_weight=0.5,
        hessian_tau=1.0,
        hessian_every=1,   # full Hessian is expensive; refresh every 5 steps
    )
