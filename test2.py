...
class NeuralEncoder:
    ...
    def _mnist_to_encoder_torch(img):
        # img: (28,28) torch -> flip(T) along last axis
    ...
    def _glm_rates_torch(self, S, cx, cy):
        """
        S: (28,28) torch in encoder frame.
        cx,cy: scalars (python floats) - sampled positions.
        Returns lam_on, lam_off (n_g, n_g) torch tensors with autograd through S.
        """
        sigma2 = self.t_sigma2
        diff_x = (cx + self.t_px_x.unsqueeze(0)) - self.t_xg.unsqueeze(1)  # (n_g,H)
        diff_y = (cy + self.t_px_y.unsqueeze(0)) - self.t_yg.unsqueeze(1)  # (n_g,W)
        gx_w = torch.exp(-0.5 * diff_x**2 / sigma2)
        gy_w = torch.exp(-0.5 * diff_y**2 / sigma2)
        c_raw = (gx_w @ S @ gy_w.T) / (2.0 * np.pi * sigma2)
        # detach the normalizer so we don't backprop through max()
    ...
    # particle filter (numpy) - unchanged in spirit
    ...
    def _adam_update_A(self, A_param, optimizer, D_t, samples_t, r_on_t, r_off_t,
        ...
        """Run `n_iter` Adam steps on the negative log-posterior at time t."""
        for _ in range(n_iter):
            optimizer.zero_grad()
            S = self._A_to_S_encoder_torch(D_t, A_param)            # (28,28) encoder frame

            # E_r : negative log-likelihood summed over sampled positions
            nll = 0.0
            ...
            # E_g : anchor to previous estimate (replaces Hessian-weighted prior)
            anchor = 0.5 * anchor_weight * ((A_param - A_anchor) ** 2).sum()

            # E_p : L1 sparsity + soft pixel-range penalty (smooth hinge-like via clamp)
    ...
    def decode(self, D, mean_offset=0.0, n_particles=80, n_samples=30,
        ...
        for t in range(T):
            # no anchor at t=0 (no previous estimate to anchor to)
...
if __name__ == "__main__":
    ...
    optotype = ds_val[1041][0].squeeze(0)

    sim = NeuralEncoder(dx=0.5, dy=0.5, dt=0.005, ds=0.3, D_diff=10.0)
    sim.fit(optotype, blur_sigma=0.0)
    sim.simulate_random_walk(T=1)
    sim.compute_activations(grid_range=10.0, grid_resolution=70)

    sim.decode(D, mean_offset=mean_offset,
           n_particles=50, n_samples=30,
           beta=1.0, gamma=0.1,
           adam_iter=10, lr=1e-4,
           anchor_weight=0.5)
